"use client";

import { useState } from "react";
import Image from "next/image";
import { motion } from "framer-motion";

/**
 * CLIENTS SECTION - FIRSTLOGIC META LAB (High-End Enterprise Light Platinum & Royal Blue)
 * 
 * Aesthetic:
 * - Background: Pristine Platinum & Ice-White Studio (#F8FAFC -> #FFFFFF -> #F1F5F9)
 * - Typography: Deep Sovereign Navy (#061A45) with Royal Cobalt Blue accents (#0A369D)
 * - 3D Marquee: Opposing vertical motion with elevated crisp white cards and soft sapphire shadows
 * - Full 23-partner interactive matrix expander
 */

interface ClientItem {
  name: string;
  src: string;
}

const clientLogos: ClientItem[] = [
  { name: "3ms", src: "/client/3ms.png" },
  { name: "Edhwi", src: "/client/Edhwi.svg" },
  { name: "Karikku", src: "/client/Karikku.svg" },
  { name: "American Board", src: "/client/american board.png" },
  { name: "Foodzer", src: "/client/foodzer.png" },
  { name: "Healine", src: "/client/healine.png" },
  { name: "MyVaahan", src: "/client/myvaahan.png" },
  { name: "Qaro", src: "/client/qaro.png" },
  { name: "Thara Cart", src: "/client/thara_cart.png" },
  { name: "Vivlino", src: "/client/vivlino.svg" },
  { name: "Cabco", src: "/client/cabco.jpeg" },
  { name: "ClubQ", src: "/client/clubq.jpeg" },
  { name: "Cool Talk", src: "/client/cool talk.jpeg" },
  { name: "DCost", src: "/client/dcost.jpeg" },
  { name: "Flexifold", src: "/client/flexifold.jpeg" },
  { name: "Relax Call", src: "/client/relax call.jpeg" },
  { name: "Vestido Nation", src: "/client/vestido nation.jpeg" },
  { name: "Kootukari", src: "/client/kootukari.jpeg" },
  { name: "Live to smile", src: "/client/live_to_smile.jpeg" },
  { name: "First Tap", src: "/client/first_tap.jpeg" },
  { name: "Ebotto", src: "/client/ebotto.jpeg" },
  { name: "Schosys", src: "/client/schosys.jpeg" },
  { name: "FinalFX", src: "/client/finalfx.png" },
];

const col1 = clientLogos.slice(0, 12);
const col2 = clientLogos.slice(12);

function ClientCard({ client }: { client: ClientItem }) {
  return (
    <div className="group/tile relative w-full h-[124px] sm:h-[130px] rounded-2xl bg-white border border-slate-200/90 shadow-[0_4px_16px_rgba(10,54,157,0.05)] hover:shadow-[0_12px_32px_rgba(10,54,157,0.15)] hover:border-[#0A369D] hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden flex items-center justify-center p-3 sm:p-4 cursor-pointer">
      {/* Light sheen on hover */}
      <div
        className="pointer-events-none absolute inset-0 -translate-x-full group-hover/tile:translate-x-full transition-transform duration-700 ease-in-out bg-gradient-to-r from-transparent via-[#0A369D]/10 to-transparent"
        aria-hidden="true"
      />
      <div className="relative w-full h-full flex items-center justify-center">
        <Image
          src={client.src}
          alt={`${client.name} logo`}
          fill
          className="object-contain p-2.5 transition-transform duration-300 ease-out group-hover/tile:scale-105"
          sizes="(max-width: 640px) 140px, (max-width: 1024px) 160px, 180px"
        />
      </div>
    </div>
  );
}

export default function Clients() {
  const [isPaused, setIsPaused] = useState(false);
  const [isExpanded, setIsExpanded] = useState(false);

  const clientCount = clientLogos.length;

  return (
    <section
      id="clients"
      className="relative w-full bg-gradient-to-b from-[#F8FAFC] via-[#FFFFFF] to-[#F1F5F9] text-[#061A45] pt-16 sm:pt-24 pb-20 sm:pb-28 border-t border-slate-200/90 overflow-hidden"
    >
      {/* Subtle Blueprint Grid */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.035] z-0">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="clients-light-grid" width="48" height="48" patternUnits="userSpaceOnUse">
              <path d="M 48 0 L 0 0 0 48" fill="none" stroke="#061A45" strokeWidth="1" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#clients-light-grid)" />
        </svg>
      </div>

      <div className="relative max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 z-10">
        
        {/* EXPANDED FULL GRID MODE */}
        {isExpanded ? (
          <div className="w-full">
            <div className="max-w-3xl mb-10 sm:mb-14">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 mb-4">
                <span className="w-2 h-2 rounded-full bg-[#0A369D] animate-pulse" />
                <span className="font-mono text-xs font-bold uppercase tracking-wider text-[#0A369D]">
                  # CLIENTS &amp; GLOBAL DEPLOYMENTS
                </span>
              </div>

              <h2 className="font-['Space_Grotesk',sans-serif] text-4xl sm:text-6xl md:text-7xl font-bold uppercase text-[#061A45] leading-[0.94] tracking-tight mb-5">
                Trusted by <br />
                <span className="text-[#0A369D]">
                  Serious Enterprises.
                </span>
              </h2>

              <p className="text-slate-600 text-sm sm:text-base md:text-lg leading-relaxed font-light max-w-xl mb-6">
                Engineered for multi-generational reliability at global institutional scale. Every partner backed by bespoke zero-compromise architecture.
              </p>
            </div>

            {/* Responsive Grid showing all 23 client logo cards */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3.5 sm:gap-4">
              {clientLogos.map((client, idx) => (
                <motion.div
                  key={client.name}
                  initial={{ opacity: 0, y: 16 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.35, delay: idx * 0.02, ease: "easeOut" }}
                >
                  <ClientCard client={client} />
                </motion.div>
              ))}
            </div>
          </div>
        ) : (
          /* TWO-COLUMN MOTION LAYOUT */
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 lg:gap-14 items-center">
            
            {/* LEFT-SIDE: ANIMATED 3D GALLERY */}
            <div className="order-2 lg:order-1 lg:col-span-6 w-full flex justify-center">
              <div className="gallery-viewport relative w-full max-w-[420px] sm:max-w-[460px] lg:max-w-[500px] h-[320px] lg:h-[460px] overflow-hidden py-2">
                <div className="gallery-perspective w-full h-full flex justify-center items-center gap-3 sm:gap-[13px]">
                  
                  {/* Column 1: Moves Upward (44s linear infinite) */}
                  <div className="w-[136px] sm:w-[165px] lg:w-[178px] shrink-0 flex flex-col">
                    <div
                      className="flex flex-col gap-[13px] pb-[13px] animate-column-up shrink-0"
                      style={{ animationPlayState: isPaused ? "paused" : "running" }}
                    >
                      {col1.map((client, idx) => (
                        <ClientCard key={`col1-primary-${client.name}-${idx}`} client={client} />
                      ))}
                    </div>
                    <div
                      className="flex flex-col gap-[13px] pb-[13px] animate-column-up shrink-0"
                      style={{ animationPlayState: isPaused ? "paused" : "running" }}
                      aria-hidden="true"
                    >
                      {col1.map((client, idx) => (
                        <ClientCard key={`col1-duplicate-${client.name}-${idx}`} client={client} />
                      ))}
                    </div>
                  </div>

                  {/* Column 2: Moves Downward (49s linear infinite) */}
                  <div className="w-[136px] sm:w-[165px] lg:w-[178px] shrink-0 flex flex-col">
                    <div
                      className="flex flex-col gap-[13px] pb-[13px] animate-column-down shrink-0"
                      style={{ animationPlayState: isPaused ? "paused" : "running" }}
                    >
                      {col2.map((client, idx) => (
                        <ClientCard key={`col2-primary-${client.name}-${idx}`} client={client} />
                      ))}
                    </div>
                    <div
                      className="flex flex-col gap-[13px] pb-[13px] animate-column-down shrink-0"
                      style={{ animationPlayState: isPaused ? "paused" : "running" }}
                      aria-hidden="true"
                    >
                      {col2.map((client, idx) => (
                        <ClientCard key={`col2-duplicate-${client.name}-${idx}`} client={client} />
                      ))}
                    </div>
                  </div>

                </div>
              </div>
            </div>

            {/* RIGHT-SIDE: CONTENT */}
            <div className="order-1 lg:order-2 lg:col-span-6 flex flex-col justify-center lg:pl-4 xl:pl-8">
              
              {/* Eyebrow Label */}
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 mb-5 sm:mb-6 w-fit">
                <span className="w-2 h-2 rounded-full bg-[#0A369D]" />
                <span className="font-mono text-[11px] font-bold uppercase tracking-wider text-[#0A369D]">
                  # CLIENTS &amp; PARTNERS
                </span>
              </div>

              {/* Editorial Headline */}
              <h2 className="font-['Space_Grotesk',sans-serif] text-4xl sm:text-6xl md:text-7xl lg:text-[58px] xl:text-[66px] font-bold uppercase text-[#061A45] leading-[0.95] tracking-tight mb-5 sm:mb-6">
                Trusted by<br />
                serious<br />
                <span className="text-[#0A369D]">
                  organisations.
                </span>
              </h2>

              {/* Supporting Statement */}
              <p className="text-sm sm:text-base md:text-lg text-slate-600 leading-relaxed font-normal max-w-md mb-7 sm:mb-8">
                Ambitious brands. Complex challenges.<br className="hidden sm:inline" />
                One sovereign engineering partner.
              </p>

              {/* Client Metric Counter Pill */}
              <div className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 bg-white shadow-sm w-fit">
                <div className="flex items-baseline gap-1">
                  <span className="font-['Space_Grotesk',sans-serif] text-3xl sm:text-4xl font-bold text-[#061A45]">
                    {clientCount}
                  </span>
                  <span className="text-[#0A369D] font-bold text-xl">+</span>
                </div>

                <div className="w-px h-8 bg-slate-200" aria-hidden="true" />

                <div className="font-mono text-xs tracking-wider uppercase text-slate-600 font-semibold">
                  <span>Verified Global Deployments</span>
                </div>
              </div>

            </div>

          </div>
        )}

        {/* BOTTOM CONTROLS STRIP */}
        <div className="pt-8 mt-12 sm:mt-16 lg:mt-20 border-t border-slate-200/80 flex justify-end">
          <div className="flex items-center gap-3">
            {/* Pause / Resume Toggle */}
            <button
              type="button"
              suppressHydrationWarning
              onClick={() => setIsPaused((prev) => !prev)}
              aria-pressed={isPaused}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-[#061A45] font-mono text-[10px] sm:text-[11px] tracking-[0.22em] uppercase font-semibold transition-all duration-200 active:scale-95 shadow-xs cursor-pointer"
            >
              {isPaused ? "▶ Resume Motion" : "⏸ Pause Motion"}
            </button>

            {/* Explore all 23 Toggle */}
            <button
              type="button"
              suppressHydrationWarning
              onClick={() => setIsExpanded((prev) => !prev)}
              aria-expanded={isExpanded}
              className="group inline-flex items-center gap-2.5 px-6 py-2.5 rounded-lg bg-[#061A45] hover:bg-[#0A369D] text-white font-mono text-[10px] sm:text-[11px] tracking-[0.22em] uppercase font-semibold shadow-[0_4px_16px_rgba(6,26,69,0.18)] hover:shadow-[0_6px_22px_rgba(10,54,157,0.28)] transition-all duration-200 active:scale-95 cursor-pointer"
            >
              <span>{isExpanded ? "Back to Motion View" : `Explore All ${clientCount} Partners`}</span>
              <span className="font-mono text-sm transition-transform duration-200 group-hover:translate-x-1">
                ↗
              </span>
            </button>
          </div>
        </div>

      </div>
    </section>
  );
}
